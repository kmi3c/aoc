...
# be easy, it's certainly not safe:
# there's very minimal steering and the area is covered in trees. You'll need to see which angles will take
...
test_array = test_array.map(&:chars)
input = File.open('input', 'rb').read
input_array = input.split("\n")
input_array = input_array.map(&:chars)

def count_trees(slope, right, down, draw = false)
 trees = 0
 x = 0
 y = 0
 while y <= (slope.size-1)  do
   row = slope[y].dup rescue binding.pry
   x -= row.size if x >= row.size
   if row[x] == '#'
     trees +=1
     row[x] = 'X'
   else
   ...
   puts row.join() if draw
   x += right
   y += down
 end
 trees
end
trees = count_trees(input_array,3,1)
puts "Part 1: #{trees}"
# --- Part Two ---
#
...
#
#puts [count_trees(test_array,1,1),2].to_s
#puts [count_trees(test_array,3,1),7].to_s
#puts [count_trees(test_array,5,1),3].to_s
#puts [count_trees(test_array,7,1),4].to_s
#puts [count_trees(test_array,2,2),2].to_s
#puts [count_trees(input_array,3,1),171].to_s

result = [
count_trees(input_array,1,1),
count_trees(input_array,3,1),
count_trees(input_array,5,1),
count_trees(input_array,7,1),
count_trees(input_array,1,2)
].reduce(:*)
puts "Part 2: #{result}"
