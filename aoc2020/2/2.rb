...
# some of the passwords wouldn't have been allowed by the Official Toboggan Corporate Policy that
...
# the password to be valid. For example, 1-3 a means that the password must contain a at least 1 time and
# at most 3 times.
#
...
correct = 0
input_array.each do |pp|
  range, policy, pass = pp.split(' ')
  count = pass.chars.select{|ch| ch == policy.gsub(':','') }.size
  correct += Range.new(*range.split('-').map(&:to_i)) === count ? 1 : 0
end
puts "Part 1: #{correct}"
#
# --- Part Two ---
...
# his old job at the sled rental place down the street! The Official Toboggan Corporate Policy actually
...
input_array.each do |pp|
  ...
  correct += range.map.with_index{|r,i| pass_chars[range[i] - 1] == policy }.uniq.size == 1 ? 0 : 1
end
puts "Part 2: #{correct}"
