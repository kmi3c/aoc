...
def count_valid(passports)
  ...
  passports.each do |p|
    p_map = p.gsub("\n", ' ').split(' ').map{|a| a.split(':')}
    keys = (p_map.transpose.first.sort) - ['cid']
    count +=  if @required_keys == keys
      valid_values(p_map) ? 1 : 0
    else
      0
    end
  end
...
puts [count_valid(test_array),2].to_s
puts "Part 1: #{count_valid(input_array)}"
#
# --- Part Two ---
#
...
def valid_values(passport)
  valid = {
    byr: -> (value) { value.to_i >= 1920 and value.to_i <= 2002 },
    iyr: -> (value) { value.to_i >= 2010 and value.to_i <= 2020 },
    eyr: -> (value) { value.to_i >= 2020 and value.to_i <= 2030 },
    hgt: ->(value) {
    #    If cm, the number must be at least 150 and at most 193.
    #    If in, the number must be at least 59 and at most 76.
    true
    ...
    pid: ->(value) { value =~ /\d\d\d\d\d\d\d\d\d/},
    cid: ->(_value) { true }
  }
  passport.each do |k,v|
    if valid[k.to_sym].call(v)
      next
    else
      return false
    end
...
puts [count_valid(invalid),0].to_s
puts [count_valid(valid),4].to_s

puts "Part 2: #{count_valid(input_array)}"
