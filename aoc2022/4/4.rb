...
#input = "2-4,6-8\n2-3,4-5\n5-7,7-9\n2-8,3-7\n6-6,4-6\n2-6,4-8"
input = input.strip.split("\n").map{|pair| pair.split(',').map{|r| r.split('-').map(&:to_i)}.map{|a| Range.new(*a)}}
puts "Part 1: #{
  input.reduce(0) do |count, ranges|
    first, second = ranges
    count += first.cover?(second) || second.cover?(first) ? 1 : 0
  end
}"
#
# --- Part Two ---
...
puts "Part 2: #{
  input.reduce(0) do |count, ranges|
    first, second = ranges
    # Check if any of range start cover other. That's enough. Same can be done with comparing starts with `<=`
    count += first.cover?(second.first) || second.cover?(first.first) ? 1 : 0
  end
}"
