...
# A given rucksack always has the same number of items in each of its two compartments, so the first half of
# the characters represent items in the first compartment, while the second half of the characters represent
...
# What is the sum of the priorities of those item types?
puts "Part 2: #{
  input.split("\n").each_slice(3).reduce(0) do |sum, rucksacks|
    first, second, third = rucksacks.map(&:chars)
    common = (first & second & third).first
    sum + letter_number(common)
  end
}"
