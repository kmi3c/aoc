...
# you're not climbing a mountain. As the circle of Snow Island recedes below you, an entire new landmass suddenly
...
# the small kind surrounded by water, not the floating kind. We really need to come up with a better naming
# scheme. Tell you what: if you can help me with something quick, I'll let you borrow my boat and you can go visit
...
def read_cards(input)
  input.split("\n").map{|r| c,n = r.split(':').map{|s| s.split('|')}; [c, n.map(&:split)] }
end

def calculate_scratchcards(input)
  read_cards(input).map do |card|
    yield card
  end.sum
end

def card_win_size(card)
  (card[1][0] & card[1][1]).size
end

def card_win?(card)
  card_win_size(card) > 0
end

def card_points(card)
  if card_win?(card)
    wins = (card[1][0] & card[1][1]).size
    w = wins-1
    points = 1
    w.times { points *= 2 }
    w == 0 ? 1 : points
...
test1 = calculate_scratchcards(@test) { |card| card_points(card)}
puts "Test 1: #{test1}"
if test1 == 13
  puts 'Pass.'
else
...
puts "Part 1: #{calculate_scratchcards(@input) { |card| card_points(card)}}"
#
# --- Part Two ---
#
...
def count_cards(input)
  ...
  cards.each do |card|
    instances[card[0][0]] = 1
    add_copies(card, instances, copies, cards)
  end
  failsafe = 2_000_000
  count = 0
  while copies.length > 0
    ...
    # failsafe
    #puts count +=1
    binding.pry if count > failsafe
  end
  instances.values.sum
end

def add_copies(card, instances, copies, cards)
  return unless card_win?(card)
  to_copy = cards[card_no(card)..(card_no(card) + card_win_size(card) - 1)]
  # count_copies
  copies.concat(to_copy)

end
test2 = 30
puts "Test 2: #{count_cards(@test) { |c, i, cps|  add_copies(c, i, cps) }}"
return if test2 != 30
puts "Part 2: #{count_cards(@input) { |c, i, cps|  add_copies(c, i, cps) }}"
