...
def execute(array)
  ...
  c = @ints[array[1]].send(%i[== + *][array[0]], @ints[array[2]])
  # puts "#{@ints[array[1]]}#{%i[== + *][array[0]]}#{@ints[array[2]]} == #{c}"
...
o_pointer = 0
opcode = 0
# restore `"1202 program alarm"` conditions
@ints[1] = 12
@ints[2] = 2
while opcode != 99
...
# Moved above code to `Computer` class
...
p1[1] = 12
p1[2] = 2
# Computer class works!
puts "  lib/: #{Computer.new(p1.dup).process == @ints[0]}"
nouns = (0..99).to_a
verbs = (0..99).to_a
noun = 0
verb = 0
nouns.product(verbs).each do |n, v|
  ci = ints.dup
  ci[1] = n
  ci[2] = v
  c = Computer.new(ci).process
  noun = n
  verb = v
  break if c == 19_690_720
end
