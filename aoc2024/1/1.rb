...
# so in order to save Christmas, you need to help them get fifty stars on their list before Santa takes off
# on December 25th.
#
# You haven't even left yet and the group of Elvish Senior Historians has already hit a problem:
# their list of locations to check is currently empty. Eventually, someone decides that the best place to
...
# unique number called the location ID. To make sure they don't miss anything, The Historians split into two
# groups, each searching the office and trying to create their own complete list of location IDs.
#
# There's just one problem: by holding the two lists up side by side (your puzzle input),
# it quickly becomes clear that the lists aren't very similar. Maybe you can help The Historians reconcile
# their lists?
#
# For example:
#
...
# they are. Pair up the smallest number in the left list with the smallest number in the right list, then
...
def split_to_left_and_right(input)
  input.split("\n").map{|lr| lr.split(' ').map(&:to_i)}.transpose
end

def find_distances_sum(input)
  left,right = split_to_left_and_right(input).map(&:sort)

  d = left.map.with_index do |l, i|
    dis = (right[i] - l).abs
    puts "#{[l, right[i]].to_s},#{dis}"
    dis
  end
...
test1 = find_distances_sum(@test) # 11
puts "Test 1: #{test1}"
if test1 == 2 + 1 + 0 + 1 + 2 + 5 #11
  puts 'Pass.'
else
  puts 'Test failed.'
  return
...
puts "Part 1: #{find_distances_sum(@input)}"
#
# --- Part Two ---
#
...
def find_distances_multipication(input)
  left,right = split_to_left_and_right(input)

  m = left.map.with_index do |l, i|
    mp = l * right.count(l)
    puts "#{[l, right[i]].to_s},#{mp}"
    mp
  end
...
test2 = find_distances_multipication(@test) # 31
puts "Test 2: #{test2}"
if test2 == 9 + 4 + 0 + 0 + 9 + 9 #31
  puts 'Pass.'
else
  puts 'Test failed.'
  return
...
puts "Part 2: #{find_distances_multipication(@input)}"
