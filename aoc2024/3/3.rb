...
# be ignored, even if they look like part of a mul instruction. Sequences like 
...
# similar test part 2, should result the same as in part 1
@test = "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))"

def find_mul(input)
  input.scan(/mul\((\d+),(\d+)\)/).map{|m| m.map(&:to_i)}
end

def sum_mul(input)
  find_mul(input).map{|m| m.reduce(&:*)}.sum
end

test1 = sum_mul(@test)
puts "Test 1: #{test1}"
if test1 ==  (2*4 + 5*5 + 11*8 + 8*5) # 161
  puts 'Pass.'
else
  puts 'Test failed.'
  return
...
puts "Part 1: #{sum_mul(@input)}"
#
# --- Part Two ---
# As you scan through the corrupted memory, you notice that some of the conditional statements are also still
# intact. If you handle some of the uncorrupted conditional statements in the program, you might be able to
...
# The other mul instructions function normally, including the one at the end that gets re-enabled by a do()
# instruction.
#
...
puts "Part 2: #{sum_mul_with_dos_and_donts(@input)}"
